...
const convertToAvi = async(fileToConvert)=>{
    ...
    const audioFile = path.join(output_dir,`${audioFileName}.avi`);
    try{
        await new Promise((resolve,reject)=>{
            ffmpeg().input(fileToConvert).audioCodec('libmp3lame').videoCodec('libxvid').on('end',()=>{
                resolve();
            }).on('error',(err)=>{
                console.log(err);
                reject();
